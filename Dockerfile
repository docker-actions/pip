...
ARG VERSION="18.1"
ARG REQUIRED_PACKAGES="python3-dev gcc g++"
ARG ROOTFS
...
RUN mkdir -p ${BUILD_DEBS} ${ROOTFS}/{sbin,usr/bin,usr/local/bin}
...
RUN apt-get install -yq python3-pip; \
    pip3 -v --no-cache-dir install --upgrade --root ${ROOTFS} pip==${VERSION} setuptools

RUN ln -s python3.6 ${ROOTFS}/usr/bin/python3
